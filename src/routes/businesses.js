...
import { uploadOwnerId } from '../middleware/upload.js';
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (business.owner_id_image_url) {
        const oldPath = path.join(process.cwd(), business.owner_id_image_url);
      ...
      // Store relative path
      const imageUrl = `/uploads/business-owner-ids/${req.file.filename}`;
      business.owner_id_image_url = imageUrl;
      await business.save();
      ...
      await recordAudit({
        ...
        entityId: business.id,
        userId: req.user?.sub,
        details: { owner_id_image_uploaded: true },
      });
...
router.get('/:id/owner-id-image', requireAuth, async (req, res, next) => {
  try {
    const business = await BusinessModel.findById(req.params.id);
    if (!business || !business.owner_id_image_url) {
      throw createError(404, 'Image not found');
    }

    const imagePath = path.join(process.cwd(), business.owner_id_image_url);
    if (!fs.existsSync(imagePath)) {
      throw createError(404, 'Image file not found');
    }
